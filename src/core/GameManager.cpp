...
void GameManager::init() {
  ...
  // Add initial balls
  auto fireBall = std::make_shared<FighterBall>(Vector2{200, 300}, 20.0f, RED,
                                                Element::FIRE);
  auto iceBall = std::make_shared<FighterBall>(Vector2{600, 300}, 25.0f, BLUE,
                                               Element::ICE);
  ...
  for (size_t i = 0; i < entities.size(); ++i) {
    for (size_t j = i + 1; j < entities.size(); ++j) {
      ...
      if (dist < min_dist && dist > 0.0f) {
        ...
        Vector2 normal =
            Vector2Normalize(Vector2Subtract(a->position, b->position));
        ...
        if (speed < 0) {
          // They are moving towards each other, meaning a collision just
          // occurred!
          const float restitution =
              1.0f; // 1.0 means perfectly elastic (no speed loss)
          float impulse =
              -(1.0f + restitution) * speed / 2.0f; // Assuming equal mass
